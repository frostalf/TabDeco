			...
			if(setting != null)
			{
				...
				if(!doesExist)
				{
					...
					if(TabDeco.debugMode) {
                        TabDeco.debugLogger.info("Registered external setting from " + plugin.getName() + ": " + "[" + settingName + "]");
                    }
					return true;
				}
			}
		...
		try
		{
			if(sorter != null)
			{
				...
				if(!doesExist)
				{
					...
					if(TabDeco.debugMode) {
                        TabDeco.debugLogger.info("Registered external sorter from " + plugin.getName() + ": " + "[" + sorterName + "]");
                    }
					return true;
				}
			}
		...
		try
		{
			for(int i = 0; i < TabDeco.externalSettings.size(); i++)
			{
				...
				if(setting.settingName.equals(settingName))
				{
					...
					if(TabDeco.debugMode) {
                        TabDeco.debugLogger.info("Removed external setting [" + settingName + "]");
                    }
					return true;
				}
			}
		...
		try
		{
			for(int i = 0; i < TabDeco.externalSorter.size(); i++)
			{
				...
				if(sorter.sorterName.equals(sorterName))
				{
					...
					if(TabDeco.debugMode) {
                        TabDeco.debugLogger.info("Removed external sorter [" + sorterName + "]");
                    }
					return true;
				}
			}
	...
	public static boolean registerTabDecoHandler(String handlerName, TabDecoHandler handler, TabDeco plugin, TabDeco tabDecoInstance)
	{
		try
		{
			if(TabDeco.tabDecoHandler == null)
			{
				...
				if(TabDeco.debugMode) {
                    TabDeco.debugLogger.info("Registered TabDeco handler " + handlerName + " for " + plugin.getName());
                }
			}
			else
			{
